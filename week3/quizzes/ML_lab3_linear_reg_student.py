...
def parsePoint(line):
    ...
    lableArray = line.split(",")
    if len(lableArray) < 2:
        return None
    
    return LabeledPoint(lableArray[0], lableArray[1:])

parsedSamplePoints = [parsePoint(sample) for sample in samplePoints]
...
averageTrainYear = (parsedTrainData
                   .map(lambda x: ("YEAR", x.label))
                   .reduceByKey(lambda x, y: x + y)).collect()[0][1] / parsedTrainData.count()
...
def calcRMSE(labelsAndPreds):
    ...
    """
    sumValue = labelsAndPreds.map(lambda x: squaredError(x[0], x[1])).reduce(lambda x, y: x + y)
    return math.sqrt(sumValue / labelsAndPreds.count())
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda x: getLabeledPrediction(w, x))#RDD (label, pres)
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)#a value

        #print "labelsAndPredsTrain:", labelsAndPredsTrain.take(1)
        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = trainData.map(lambda x: ("XX", gradientSummand(w, x))).reduceByKey(lambda x, y: x + y).collect()

        # Update the weights
        alpha_i = alpha / (n * np.sqrt(i+1))
        w -= alpha_i * gradient[0][1]
    return w, errorTrain
...
averageValYear = (parsedValData
                   .map(lambda x: ("VAL", x.label))
                   .reduceByKey(lambda x, y: x + y)).collect()[0][1] / parsedValData.count()

labelsAndPreds = parsedValData.map(lambda x: (x.label, averageValYear))
...
def twoWayInteractions(lp):
    ...
    """
    newFeature = []
    for first in lp.features:
        newFeature.append(first)
        for second in lp.features:
            newFeature.append(first * second)
    return LabeledPoint(lp.label, newFeature)
